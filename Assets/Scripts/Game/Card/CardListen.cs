...
public class CardListen : MonoBehaviour, IPointerExitHandler, IPointerEnterHandler
{
	...
	public IsHasDragTarget isDragTarget;
	...
	private void Awake()
	{
		...
		if (isDragTarget == IsHasDragTarget.DragNoTarget)
		{
			...
			{
				Debug.Log(gameObject + "没有添加DragNoTarget拖拽脚本");
			}
		}
		else if (isDragTarget == IsHasDragTarget.DragHasTarget)
		{
			...
			{
				Debug.Log(gameObject + "没有添加DragHasTarget拖拽脚本");
			}
		}
		else
		{
			Debug.Log(gameObject + "没有添加拖拽脚本");
		}
